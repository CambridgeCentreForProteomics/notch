...
plot_correlation <- function(obj, colours=get_cat_palette(7), include_tls=FALSE){
  ...
  p_cor <- cor(x=obj$pred, y=obj$obs, method='pearson') %>% round(digits=3)
  
  pred_vs_obs_tag_int <- obj %>%
    ggplot(aes(log2(pred), log2(obs))) +
    geom_point(size=0.05, alpha=0.02, colour='grey30') +
    ...
    ylab('Observed intensity x(log2)') +
    ...
    annotate(geom='text', x=10, y=max(log2(obj$obs)),
             label=paste0('r = ', p_cor), size=5)
...
pred_obs %>% names() %>% lapply(function(x){
  ...
  p1 <- plots$pred_vs_obs_tag_int
  p2 <- plots$pred_vs_obs_tag_int_per_sample 
  
  ggsave(sprintf('../results/plots/%s_pred_obs.png', x), p1, width=4, height=4)
  ggsave(sprintf('../results/plots/%s_pred_obs_per_tag.png', x), p2, width=6, height=6)
  
  print(p1 + ggtitle(x))
  print(p2 + ggtitle(x))
})
```
Replotting a LOPIT-DC replicate 1 using density 
```{r}
colours=get_cat_palette(7)
obj <- pred_obs$LOPIT_DC_U2OS_Rep1 %>%
  filter(is.finite(obs), is.finite(pred))
...
pred_bias <- pred_obs %>% names() %>% lapply(function(x){
  ...
  p1 <- plots$p_pred_bias
  p2 <- plots$p_pred_bias_per_sample
  
  ggsave(sprintf('../results/plots/%s_pred_under.png', x), p1, width=4, height=4)
  ggsave(sprintf('../results/plots/%s_pred_under_per_tag.png', x), p2, width=6, height=6)
...
for(name in names(pred_bias)){
  ...
  ggsave(sprintf('../results/plots/%s_pred_missing.png', name), p, width=4, height=4)
  
  print(p + ggtitle(name))
}
```
