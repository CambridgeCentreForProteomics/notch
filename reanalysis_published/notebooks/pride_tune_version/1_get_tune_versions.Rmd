...
abstract: | 
  Here, we extract the Tune versions from Orbitrap Fusion, Lumos and Eclipse studies
output:
  pdf_document:
...
Functions to download data and extract Tune version.
```{r}
...
Read in the table of orbitrap fusion pride accessions and extract metainfo. We'll
also set up a cache so we don't need to reprocess the same files in case this doesn't
...
submission_details <- submission_details %>%
  filter(!grepl(';', Instrument),
         grepl('Orbitrap', Instrument),
         Repos=='PRIDE') %>%
...
tune_version_table <- readRDS(cache_filepath) %>% bind_rows() %>%
  ...
  merge(submission_details, by.x='PRIDE', by.y='Dataset.Identifier') %>%
  select(PRIDE, `Instrument model`, `Software version`, year) %>% unique() %>%
  separate(`Software version`, into=c('major', 'minor'), sep='\\.', remove=FALSE)
...
```
Tune versions by submission year
```{r}
library(camprotR)
#info_details
...
```
Tune versions by data generation year
```{r}
...
Why don't some datasets get parsed?
```{r}
