...
```{r}
prot_sum %>% lapply(function(x) lapply(x, dim))
```
...
How many proteins per PSM filtering and AGC target when using sum summarisation
```{r}
prot_robust %>% lapply(function(x) lapply(x, dim))
```
...
Testing out function to plot protein intensity vs fold change
```{r}
x <- quant_vs_mean_prot_sum$`filtered, inc S/N`$`AGC: 2E5`
y <- notch_per_protein$`filtered, inc S/N`$`AGC: 2E5` %>% mutate(sample=remove_x(sample))

to_plot <- merge(x, y, by.x=c('id', 'tag'), by.y=c('Master.Protein.Accessions', 'sample')) %>%
...
print(p + geom_point(size=0.2, alpha=0.1, colour='grey30'))
```

```{r}
suppressWarnings(print(p %+% to_plot[to_plot$fraction_below>0,] + aes(fraction_below) + geom_point(size=0.5, alpha=0.5) +
        geom_smooth()))
```
...
```{r}
saveRDS(quant_vs_mean_prot_sum, '../results/quant_vs_mean_prot_sum.rds')
saveRDS(quant_vs_mean_prot_robust, '../results/quant_vs_mean_prot_robust.rds')
```
